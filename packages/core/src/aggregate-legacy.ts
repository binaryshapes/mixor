...
> {
  ...
  ): Result<SchemaValues<F>, SchemaErrors<F>>;
...
>(config: StateManagerConfig<F, E, S>): AggregateStateManager<F, E, S> {
  let state = { ...(config as Any).initialState };
  ...
  const baseManager = {
    ...
    set<K extends keyof SchemaValues<F> & keyof F>(
      ...
      if (fieldSchema && typeof fieldSchema === 'function') {
        const validationResult = fieldSchema(value) as Result<void, SchemaErrors<F>>;
...
>(schema: Schema<F>, eventStore?: EventStore<E>, specifications?: SP) {
  return <M extends MethodsBuilder<F, E, S, SP>>(builder: M) => {
    ...
    type AggregateOptions = { checkSpecs?: boolean };
...
type InferAggregate<A> = A extends (...args: Any[]) => Result<infer R, Any> ? R : never;

export type { InferAggregate };
export { aggregate };
