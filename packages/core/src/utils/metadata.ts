...
/**
 * The error thrown when a metadata error occurs.
 */
 ...
 * console.log(getMetadata(obj)); // { name: 'Jane' }
 *
...
function setMetadata<M extends ObjectMetadata, O extends object = object>(
  ...
  if (!originalWritable) {
    throw MetadataError('Metadata already exists, cannot be overwritten');
  }
...
function getMetadata<M, O extends object = object>(o: O): M {
  ...
  throw MetadataError('No metadata found');
}
