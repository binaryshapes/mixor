 ...
 * Result with code 'NOT_LESS_THAN'.
 *
...
const Lt = rule((threshold: number) =>
  n.assert((value: number) => value < threshold, 'NOT_LESS_THAN')
);
...
n.info(Lt)
  ...
  .doc({
    ...
    threshold, the rule will return a failure Result with code 'NOT_LESS_THAN'.
    `,
  });
