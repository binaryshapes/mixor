...
interface EnumerateFunction {
   ...
   * ```
   */
  <T extends string | number>(values: T[]): (value: T) => Result<T, 'INVALID_ENUM_VALUE'>;
}
 ...
 * @public
 */
const enumerate: EnumerateFunction = <T extends string | number | Record<string, string | number>>(
 ...
 * @internal
 */
const enumerateArray = <T extends string | number>(
  values: T[],
